...
void getContainerFromFile(Container *container) {
    while (!input_file_stream.eof()) {
        try {
            double fuel_consumption;
            int type, fuel_tank_capacity;
            input_file_stream >> type >> fuel_tank_capacity >> fuel_consumption;

            switch (type) {
                case Vehicle::type::TRUCK:
                    int carrying_capacity;
                    input_file_stream >> carrying_capacity;
            ...
            std::cout << "something went wrong while reading from file \n\t" << e.what() << std::endl;
            break;
        }
    }
}

// Заполнение контейнера заданным числом случайных элементов.
void getContainerRandom(Container *container, int randomSize) {
    for (int i = 0; i < randomSize; ++i) {
        auto type = rand() % 3;
        auto fuel_tank_capacity = rand() % 10000 + 1;
        auto fuel_consumption = rand() % 1000 + 1;

        switch (type) {
            case Vehicle::type::TRUCK:
                container->append(Truck(fuel_tank_capacity, fuel_consumption, rand() % 100 + 1));
                break;
            case Vehicle::type::BUS:
                container->append(Bus(fuel_tank_capacity, fuel_consumption, short(rand() % 30 + 1)));
                break;
            case Vehicle::type::CAR:
                container->append(Car(fuel_tank_capacity, fuel_consumption, short(rand() % 340 + 1)));
                break;
            default:
                break;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (flags_positions[RANDOM]) {
        ...
        getContainerRandom(&_container, random_size);
        ...
        getContainerFromFile(&_container);
        input_file_stream.close();
    }

    // Вывод содержимого контейнера в файл
    output_file_stream << "Filled container:\n";
    output_file_stream << _container.toString();
